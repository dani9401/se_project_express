...
const createUser = (req, res) => { ////////////////////////tested on postman
  console.log(req)
  ...
  .catch((err) => {
   ...
   if  (err.name === "ValidationError") {
    ...
    res.status(500).send({message: "An error has occurred on the server.", err}) //////need proper 500 error message
   }
  })
}
...
const getUser = (req, res) => {
  ...
  .catch((err) => {
    ...
    if  (err.name === "CastError") {
      res.status(404).send({message: "No user with that ID. Please try again.", err})
    } else {
    res.status(500).send({message: "An error has occurred on the server.", err})} //////need proper 500 error message
})
};
